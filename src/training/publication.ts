...
export class HitchModelPublisher implements ModelPublisher {
  constructor(readonly store: TrainingContentStore, readonly options: { command: string[]; root: string; python: string[]; activationPath: string }) {}
  async activate(model: ModelVersion, activationId: string) {
    ...
    const imported = await jsonProcess(this.options.command, ['--root', this.options.root, 'models', 'add', directory, '--name', model.id.slice(7), '--json'], undefined, 3_600_000) as { model_id: string; tokenizer_digest: string; template_digest: string }
    ...
    await atomicWrite(this.options.activationPath, { schemaVersion: 1, activationId, modelVersionId: model.id, modelRef: await this.store.putJson(model),
      hitchModel: `local/${imported.model_id}` })
