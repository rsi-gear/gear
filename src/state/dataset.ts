...
async function localTreeDigest(root: string): Promise<string> {
  const records: string[] = []
  async function visit(path: string): Promise<void> {
    const info = await lstat(path)
    const name = relative(root, path).split(sep).join('/') || '.'
    if (info.isSymbolicLink()) throw new Error(`dataset identity refuses symlink: ${name}`)
    if (info.isDirectory()) {
      records.push(`d\0${name}`)
      for (const entry of (await readdir(path)).sort()) await visit(join(path, entry))
      return
    }
    if (!info.isFile()) throw new Error(`dataset identity refuses special file: ${name}`)
