...
export interface LlmVerifierRuntimeIdentity {
  pythonVersion: string
  packageVersion: string
  packageIntegrity: string
}

export interface LlmVerifierAssessorConfig {
  pythonExecutable: string
  runtime: LlmVerifierRuntimeIdentity
  model: string
...
function parseRuntimeIdentity(value: unknown): LlmVerifierRuntimeIdentity {
...
function parseConfig(ref: ComponentRef<unknown>): LlmVerifierAssessorConfig {
  ...
  if (typeof config.model !== 'string' || config.model.length === 0) throw new TypeError('llm-verifier model is required')
  ...
  return {
    ...
    passEnv: [...config.passEnv] as string[],
  }
}

function selectionEnvironment(names: readonly string[]): NodeJS.ProcessEnv {
  ...
  for (const name of names) {
    ...
    if (value === undefined) throw new Error(`llm-verifier environment variable is not set: ${name}`)
    env[name] = value
  }
  return env
}
...
function redactEnvironmentValues(text: string, names: readonly string[]): string {
  ...
  for (const name of names) {
    const value = process.env[name]
    if (value !== undefined && value.length > 0) redacted = redacted.split(value).join('[REDACTED]')
  }
  return redacted
}
...
function runProcess(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', code => {
      ...
      if (overflow !== undefined) return reject(new Error(`llm-verifier ${overflow} exceeded ${maxOutputBytes} bytes`))
...
export async function resolveLlmVerifierRuntime(
  pythonExecutable: string,
  signal: AbortSignal = AbortSignal.timeout(30_000),
): Promise<LlmVerifierRuntimeIdentity> {
  if (!isAbsolute(pythonExecutable)) throw new TypeError('llm-verifier pythonExecutable must be absolute')
  const result = await runProcess(
    ...
    signal,
    64 * 1024,
    { PYTHONDONTWRITEBYTECODE: '1', PYTHONNOUSERSITE: '1', PYTHONUTF8: '1', LC_ALL: 'C' },
  )
  if (result.exitCode !== 0) throw new Error(`failed to inspect llm-verifier runtime: ${result.stderr.slice(-4000)}`)
  return parseRuntimeIdentity(JSON.parse(result.stdout) as unknown)
}
...
function exactCandidateKeys(value: Record<string, unknown>, candidateIds: readonly string[], label: string): void {
  ...
  if (JSON.stringify(keys) !== JSON.stringify(expected)) throw new Error(`${label} candidate identity mismatch`)
}

function parseBridgeResult(value: unknown, candidateIds: readonly string[]): {
  cells: BridgeCell[]
  usage: CandidateAssessmentUsage
} {
  const result = record(value, 'llm-verifier bridge result')
  if (result.schema_version !== 1 || !Array.isArray(result.cells)) throw new Error('invalid llm-verifier bridge result schema')
  const cells = result.cells.map((cellValue, index) => {
    const cell = record(cellValue, `llm-verifier cells[${index}]`)
    if (typeof cell.task_name !== 'string' || cell.task_name.length === 0) throw new Error('llm-verifier task name is invalid')
    ...
    if (Object.values(parsedScores).some(score => score < 0 || score > 1)) throw new Error('llm-verifier score is outside [0, 1]')
...
export class LlmVerifierCandidateAssessor implements CandidateAssessor {
  readonly ref: ComponentRef<unknown>
  readonly config: LlmVerifierAssessorConfig
  ...
  async assess(
    ...
    if (context.trajectoryReader === undefined) throw new Error('llm-verifier assessor requires a trajectory reader')
    const candidates = [...request.candidates].sort((left, right) => left.candidateId.localeCompare(right.candidateId))
    if (candidates.length === 0) throw new Error('llm-verifier assessor requires candidates')
    const candidateIds = candidates.map(candidate => candidate.candidateId)
    const trialMaps = candidates.map(candidate => {
      const trials = new Map<string, typeof candidate.seedEvaluation.trials[number]>()
      for (const trial of candidate.seedEvaluation.trials) {
        ...
        if (trials.has(key)) throw new Error(`llm-verifier candidate ${candidate.candidateId} has duplicate trial cell ${key}`)
    ...
    const cellKeys = [...trialMaps[0]!.keys()]
      ...
      .sort()
    if (cellKeys.length === 0) throw new Error('llm-verifier candidates have no common valid paired rollout cells')
    const bridgeCells = []
    const evidenceCells: Array<Record<string, JsonValue>> = []
    for (const key of cellKeys) {
      const candidateTraces = []
      let sharedProblem: string | undefined
      let sharedProblemDigest: string | undefined
      let taskName = ''
      let attempt = 0
      const evidenceCandidates: Array<Record<string, JsonValue>> = []
    ...
    const processResult = await runProcess(
      ...
      selectionEnvironment(this.config.passEnv),
    )
    ...
    if (bridge.cells.length !== evidenceCells.length) throw new Error('llm-verifier bridge omitted task cells')
    ...
    for (let index = 0; index < bridge.cells.length; index += 1) {
      ...
      for (const candidateId of candidateIds) totals[candidateId] = totals[candidateId]! + cell.scores[candidateId]!
    ...
    if (divisor === 0) throw new Error('llm-verifier assessor received no task cells')
    ...
    const candidateMetrics: Record<string, MetricSet> = Object.fromEntries(candidates.map(candidate => [
