...
import { evaluatedFixture, fixtures, revise, scopeFixture, settings, MemorySearchStore } from '../../src/search/testing.js'
...
it('applies the old time check to a zero-cost process reservation before any projection effect', async () => {
  const root = mkdtempSync(join(tmpdir(), 'gepa-process-budget-')); roots.push(root)
  const now = Date.now()
  ...
  const harness = artifacts.putJson({ commitOid: fixture.anchor.commit,
    manifestDigest: fixture.anchor.manifestDigest }, 'harness.directory.v1')
  let physicalCalls = 0
  fixture.provider.completeProcess = async () => { physicalCalls++; throw new Error('unexpected physical projection') }
  const provider = new GepaProcessCompletionProvider(join(root, 'operations'), artifacts, bindings, fixture.provider)
  const limits = settings().budgets
  limits.round.timeoutMs = 50
  ...
  const input: GepaProcessCompletionInput = { roundIdentity: { evolutionId: 'e', roundId: 'r' }, repairId: 'repair-a',
    ...
    cachedRef: artifacts.putJson({ schemaVersion: 1, cells: [] }, 'gepa.cached-cells.v1'),
    baseKey, cellRef: artifacts.putJson(base as unknown as JsonValue, 'gepa.evidence-cell.v1'),
    deadlineAt: now + 60_000, budgetCut, roundStartedAt: now - 100 }
  const operationId = digestJson(['process-budget', root]).slice(7)
  const envelope: OperationEnvelope = { campaignId: 'search-r', decisionIndex: 0, localKey: 'process',
    operationId, idempotencyKey: operationId, kind: 'gepa.process-complete',
    input: input as unknown as JsonValue, inputDigest: jsonDigest(input as unknown as JsonValue),
    implementationDigest: provider.describe().implementationDigest, bindingSetRef: bindings.create({ harness }),
  ...
  if (submitted.status !== 'completed' || submitted.completion.outcome.kind !== 'result')
    throw new Error('process denial did not seal a result')
  ...
  expect(artifacts.getJson(value.executionRef)).toMatchObject({ schemaVersion: 1, cells: [],
    failure: { kind: 'budget-exhausted', code: 'time', message: 'search budget exhausted: time' } })
  const onTime: GepaProcessCompletionInput = { ...input, roundStartedAt: now }
  const onTimeId = digestJson(['on-time-process-budget', root]).slice(7)
  const onTimeEnvelope: OperationEnvelope = { ...envelope, operationId: onTimeId, idempotencyKey: onTimeId,
    input: onTime as unknown as JsonValue, inputDigest: jsonDigest(onTime as unknown as JsonValue) }
  expect(await provider.prepareForDispatch(onTimeEnvelope, context)).toEqual({ startsBudgetClock: true })
  expect(physicalCalls).toBe(0)
  expect(await provider.cancel(onTimeEnvelope)).toMatchObject({ status: 'cancelled', releaseConfirmed: true })
})
