...
export class HitchEvaluationError extends Error {
    code;
    failedEvidence;
    constructor(message, code = 'hitch_evaluation_failed', failedEvidence) {
        super(message);
        this.code = code;
...
function verifierArtifact(value, label) {
...
function verifierDiagnostics(value) {
    const diagnostics = record(value, 'verifier evidence diagnostics');
    exactFields(diagnostics, ['ctrf', 'stdout', 'stderr', 'infrastructure_error', 'retry_history'], 'verifier evidence diagnostics');
    const ctrf = diagnostics.ctrf === undefined
        ? undefined
        : verifierArtifact(diagnostics.ctrf, 'verifier evidence diagnostics.ctrf');
    const artifactArray = (field) => {
        ...
        return source.map((item, index) => verifierArtifact(item, `verifier evidence diagnostics.${field}[${index}]`));
    };
    ...
    if (diagnostics.retry_history !== undefined) {
        ...
        retryHistory = diagnostics.retry_history.map((item, index) => jsonValue(item, `verifier evidence diagnostics.retry_history[${index}]`));
    }
...
function evaluationTrialScores(value, label) {
...
function verifierTrajectoryRefs(value, label) {
    if (!Array.isArray(value))
        throw new HitchEvaluationError(`${label} must be an array`, 'invalid_hitch_result');
    return value.map((item, index) => {
        ...
        if (!/^run_[0-9a-f]{32}$/u.test(runId))
            throw new HitchEvaluationError(`${label}[${index}].run_id is invalid`, 'invalid_hitch_result');
...
function verifierProcessEvidence(value) {
    ...
    if (!Array.isArray(process.components))
        throw new HitchEvaluationError('verifier process evidence.components must be an array', 'invalid_hitch_result');
...
function verifierFeedback(value, process) {
    ...
    const items = feedback.items.map((item, index) => {
        ...
        if (ids?.some(id => !componentIds.has(id)))
            throw new HitchEvaluationError('verifier feedback references an unknown process component', 'invalid_hitch_result');
...
function verifierStructuredArtifact(value, name) {
    ...
    const expected = `verifier/${name}.json`;
    if (artifact.ref !== expected)
        throw new HitchEvaluationError(`verifier structured artifact ${name}.ref is invalid`, 'invalid_hitch_result');
    return {
        ref: expected,
...
function redactions(value, label) {
    if (!Array.isArray(value))
        throw new HitchEvaluationError(`${label} must be an array`, 'invalid_hitch_result');
...
function projectedJsonValue(value, label) {
    if (value === null || typeof value === 'string' || typeof value === 'boolean')
        return value;
    if (typeof value === 'number' && Number.isFinite(value))
        return value;
    if (Array.isArray(value))
        return value.map((item, index) => projectedJsonValue(item, `${label}[${index}]`));
    if (typeof value !== 'object') {
        throw new HitchEvaluationError(`${label} must be JSON`, 'invalid_hitch_result');
    }
    const source = value;
...
function surfaceNode(value, index, runId, eventCount) {
    ...
    if (seq >= eventCount)
        throw new HitchEvaluationError(`${label}.seq is out of range`, 'invalid_hitch_result');
...
function validateProjectedSources(value, runId, label) {
    if (Array.isArray(value)) {
    ...
    for (const [key, child] of Object.entries(item))
        validateProjectedSources(child, runId, `${label}.${key}`);
}
function requestBoundary(value, index, runId, eventCount, nodeCount) {
    ...
    if (attempt < 0)
        throw new HitchEvaluationError(`${label}.attempt must be non-negative`, 'invalid_hitch_result');
    const retryId = boundary.retry_id === undefined
        ? undefined
        : projectedJsonValue(boundary.retry_id, `${label}.retry_id`);
    if (retryId !== undefined)
        validateProjectedSources(retryId, runId, `${label}.retry_id`);
    return {
...
function hitchContentExcerpt(value, label, runId) {
...
function chunkSummary(value, index, runId, eventCount) {
    ...
    if (summary.partial !== undefined) {
        ...
        if (raw.status !== 'incomplete')
            throw new HitchEvaluationError(`${label}.partial.status is invalid`, 'invalid_hitch_result');
        ...
        if (raw.streams === undefined) {
            ...
            const streams = raw.streams.map((item, streamIndex) => {
                ...
                return {
                    blockIndex, blockStartSeq, kind: stream.kind, content,
    ...
    if (usage !== undefined)
        validateProjectedSources(usage, runId, `${label}.usage`);
    if (finishReason !== undefined)
        validateProjectedSources(finishReason, runId, `${label}.finish_reason`);
    return {
...
function trialSlots(value, label) {
    if (!Array.isArray(value))
        throw new HitchEvaluationError(`${label} must be an array`, 'invalid_hitch_result');
    return value.map((item, index) => {
        ...
        if (attempt <= 0)
            throw new HitchEvaluationError(`${label}[${index}].attempt must be positive`, 'invalid_hitch_result');
...
function memorySizeBytes(value) {
...
export class HitchCliEvaluator {
    options;
    repositoryPath;
    executablePathPromise;
    trajectoryCache = new Map();
    trajectoryDigestByRun = new Map();
    trajectoryLoads = new Map();
    trajectoryCacheBytes = 0;
    get controlPlane() {
    ...
    constructor(options) {
        ...
        if (options.executable.length === 0)
            throw new TypeError('hitch.executable must not be empty');
        if (!/^[a-z0-9][a-z0-9_-]*$/u.test(options.harnessId))
            throw new TypeError('hitch.harnessId is invalid');
        if (!Number.isSafeInteger(options.attempts) || options.attempts <= 0)
            throw new TypeError('hitch.attempts must be a positive integer');
        if (!Number.isSafeInteger(options.maxConcurrent) || options.maxConcurrent <= 0)
            throw new TypeError('hitch.maxConcurrent must be a positive integer');
        if (!Number.isSafeInteger(options.setupTimeoutMs) || options.setupTimeoutMs < 0)
            throw new TypeError('hitch.setupTimeoutMs must be non-negative');
        if (!Number.isSafeInteger(options.terminationGraceMs) || options.terminationGraceMs < 0)
            throw new TypeError('hitch.terminationGraceMs must be non-negative');
        if (!Number.isSafeInteger(options.maxOutputBytes) || options.maxOutputBytes <= 0)
            throw new TypeError('hitch.maxOutputBytes must be positive');
        ...
        if (options.passEnv.some(name => !/^[A-Z_][A-Z0-9_]*$/u.test(name)))
            throw new TypeError('hitch.passEnv contains an invalid environment variable name');
        const controlPlane = this.controlPlane;
        if (controlPlane.mode !== 'direct' && controlPlane.mode !== 'daemon')
            throw new TypeError('hitch.controlPlane.mode is invalid');
    ...
    async checkVersion(signal) {
        ...
        const timeout = setTimeout(() => controller.abort(new HitchEvaluationError(`Hitch version check timed out for ${this.options.executable}; Gear requires agent-hitch >= 0.2.5`, 'hitch_version_check_failed')), 5_000);
        let result;
        try {
            result = await this.run(['--version'], this.repositoryPath, controller.signal, 16_384);
        }
        finally {
        ...
        if (!supported) {
            ...
            throw new HitchEvaluationError(`unsupported Hitch CLI ${match[0].trim()}; Gear requires agent-hitch >= ${minimum}${this.daemonMode ? ' for daemon eval control-plane support' : ' for stable eval identity and multi-attempt rerun'}`, 'unsupported_hitch_version');
        }
        return sha256(output);
    }
    async checkDaemon() {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(new HitchEvaluationError(`Hitch daemon status check timed out for root ${this.options.root || '<default>'}`, 'hitch_daemon_check_failed')), 5_000);
        let result;
        try {
        ...
        if (result.exitCode !== 0 || health.schema_version !== '1' || health.status !== 'running') {
            throw new HitchEvaluationError(`Hitch daemon is not running for root ${this.options.root || '<default>'}: ${result.stderr.slice(-4000)}`, 'hitch_daemon_unavailable');
        }
    ...
    prepareSubmission(round, request) {
        if (!this.daemonMode)
            return undefined;
    ...
    async reserve(round, request, signal, intent) {
        signal?.throwIfAborted();
        // A stored intent must remain recoverable even if the configured mode changed.
        if (intent !== undefined)
            return this.reserveDaemonEvaluation(round, intent, signal);
        if (this.daemonMode)
            throw new TypeError('Hitch daemon submission requires a persisted submission intent');
        return { provider: 'hitch-cli', evalId: `eval_${randomUUID().replaceAll('-', '')}` };
    }
    async reserveDaemonEvaluation(round, intent, signal) {
        ...
        const result = await this.run(args, round.workspaceRoot, signal === undefined ? timeout : AbortSignal.any([signal, timeout]), 64 * 1024);
        if (result.exitCode !== 0) {
        ...
        if (accepted.schema_version !== '1' || !/^eval_[0-9a-f]{32}$/u.test(evalId)
            || !new Set(['queued', 'running', 'cancelling', 'cancelled', 'succeeded', 'failed']).has(accepted.status)) {
    ...
    submissionParameters(intent) {
        ...
        if (typeof parameters.root !== 'string')
            throw new TypeError('Hitch submission root is invalid');
        return { root: parameters.root, args: stringArray(parameters.args, 'Hitch submission arguments') };
    }
    async recoverReservation(round, _request, signal, intent) {
        const parameters = this.submissionParameters(intent);
        try {
            return await this.reserveDaemonEvaluation(round, intent, signal);
        }
            ...
            if (list.schema_version !== '1' || !Array.isArray(list.evals))
                throw submissionError;
            for (const value of list.evals) {
                ...
                if (!/^eval_[0-9a-f]{32}$/u.test(evalId))
                    throw submissionError;
                const inspection = await this.inspectEvaluation(evalId, this.repositoryPath, signal, 'hitch_eval_recovery_inspect_failed', parameters.root);
                if (inspection.submission === undefined)
                    continue;
    ...
    async cancelReservation(reservation, intent) {
    ...
    assertEvaluationRequest(round, request) {
        if (!isExactGitCommit(request.harnessRef))
            throw new TypeError('Hitch evaluation requires a full Git commit OID');
        if (request.dataset.length === 0)
            throw new TypeError('Hitch evaluation dataset must not be empty');
        ...
        if (request.condition.seeds !== undefined)
            throw new TypeError('Hitch CLI adapter does not support typed rollout seeds');
    ...
    daemonIdempotencyKey(round, request) {
    ...
    parseResourceVector(value, label) {
    ...
    async evaluationIdentity(round, request, signal) {
        signal?.throwIfAborted();
        ...
        if (await this.hasStandardBenchmarkManifest(round, request)) {
            ...
            signal?.throwIfAborted();
            if (datasetDigest !== request.condition.dataset.digest)
                return undefined;
        }
        return this.resolveEvaluationIdentity(round, request, signal);
    }
    async hasStandardBenchmarkManifest(round, request) {
        try {
    ...
    async resolveEvaluationIdentity(round, request, signal) {
        signal?.throwIfAborted();
    ...
    async inspectCapabilities(signal) {
        ...
        if (processResult.exitCode !== 0) {
            ...
            throw new HitchEvaluationError(`Hitch bounded trajectory capabilities are unavailable: ${excerpt(output, 1_000)}`, 'hitch_trajectory_capabilities_unavailable');
        }
        let parsed;
        try {
        ...
        if (value.schema_version !== '1' || value.trajectory_analysis !== '1' || value.trajectory_events_page !== '1') {
            throw new HitchEvaluationError('Hitch must provide trajectory_analysis=1 and trajectory_events_page=1', 'hitch_trajectory_capabilities_unavailable');
        }
        ...
        return {
            ...
            ...(value.verifier_evidence === '1' ? { verifierEvidence: 1 } : {}),
        };
    }
    async inspectTrajectoryAnalysis(runId, signal) {
        if (!/^run_[0-9a-f]{32}$/u.test(runId))
            throw new TypeError('Hitch trajectory requires a valid run ID');
        signal.throwIfAborted();
        ...
        if (pending !== undefined)
            return this.waitForTrajectoryLoad(runId, pending, signal);
        const controller = new AbortController();
        const shared = {
        ...
        void shared.promise.then(analysis => {
            shared.settled = true;
            this.cacheTrajectory(analysis);
            if (this.trajectoryLoads.get(runId) === shared)
                this.trajectoryLoads.delete(runId);
        }, () => {
            shared.settled = true;
            if (this.trajectoryLoads.get(runId) === shared)
                this.trajectoryLoads.delete(runId);
    ...
    async inspectTrajectoryEvents(runId, query, signal) {
        ...
        if (processResult.exitCode !== 0)
            throw this.trajectoryCommandError('events', runId, processResult);
        let parsed;
        try {
            parsed = JSON.parse(processResult.stdout);
        }
    ...
    async inspectVerifierEvidence(runId, signal) {
        if (!/^run_[0-9a-f]{32}$/u.test(runId))
            throw new TypeError('Hitch verifier evidence requires a valid run ID');
        signal.throwIfAborted();
        const args = [
            ...(this.options.root.length === 0 ? [] : ['--root', this.options.root]),
            'verifier', 'inspect', runId, '--json',
        ];
        const processResult = await this.run(args, this.repositoryPath, signal, this.options.maxTrajectoryOutputBytes);
        if (processResult.exitCode !== 0) {
            const output = `${processResult.stderr}\n${processResult.stdout}`.trim();
            ...
            if (output !== 'hitch: unknown command: verifier') {
                throw new HitchEvaluationError(`Hitch verifier inspect failed with exit ${processResult.exitCode}: ${excerpt(output, 1_000)}`, 'hitch_verifier_inspect_failed');
            }
            return {
                runId,
                verifier: {
        ...
        exactFields(verifierRecord, ['status', 'result', 'result_sha256', 'scores', 'process', 'feedback', 'structured_artifacts', 'diagnostics', 'issues'], 'verifier evidence verifier');
    ...
    waitForTrajectoryLoad(runId, shared, signal) {
        signal.throwIfAborted();
        shared.waiters += 1;
        return new Promise((resolvePromise, reject) => {
            ...
            void shared.promise.then(analysis => { release(); resolvePromise(analysis); }, error => { release(); reject(error); });
        });
    }
    async fetchTrajectoryAnalysis(runId, signal) {
        ...
        const processResult = await this.run(args, this.repositoryPath, signal, maxBytes);
        if (processResult.exitCode !== 0)
            throw this.trajectoryCommandError('project', runId, processResult);
        let parsed;
        try {
            parsed = JSON.parse(processResult.stdout);
        }
    ...
    parseTrajectoryAnalysis(value, runId) {
        ...
        const events = result.events.map((event, index) => {
            ...
            if (seq >= eventCount)
                throw new HitchEvaluationError(`trajectory analysis events[${index}].seq is out of range`, 'invalid_hitch_result');
            return projected;
        });
        if (events.some((event, index) => index > 0
            && record(events[index - 1], 'trajectory analysis event').seq >= record(event, 'trajectory analysis event').seq)) {
    ...
    parseTrajectoryEventsPage(value, runId, query, limit) {
        ...
        if (!Array.isArray(result.events))
            throw new HitchEvaluationError('trajectory events events must be an array', 'invalid_hitch_result');
        if (result.events.length > limit)
            throw new HitchEvaluationError('trajectory events page exceeds requested limit', 'invalid_hitch_result');
        const events = result.events.map((event, index) => {
        ...
        if (events.some((event, index) => index > 0
            && record(events[index - 1], 'trajectory event').seq >= record(event, 'trajectory event').seq)) {
        ...
        if (totalMatches < events.length)
            throw new HitchEvaluationError('trajectory events total_matches is inconsistent', 'invalid_hitch_result');
    ...
    trajectoryCommandError(action, runId, result) {
        const output = `${result.stderr}\n${result.stdout}`.trim();
        const structured = structuredCliError(output);
        const stableCode = structured?.code
            ?? output.match(/(?:^|\s)(trajectory_[a-z0-9_]+):/u)?.[1]
            ?? `hitch_trajectory_${action}_failed`;
        return new HitchEvaluationError(`Hitch trajectory ${action} failed for ${runId}: ${excerpt(structured?.message ?? output, 2_000)}`, stableCode);
    }
    cacheTrajectory(analysis) {
        ...
        if (previousDigest !== undefined) {
            ...
            if (previous !== undefined)
                this.trajectoryCacheBytes -= previous.bytes;
        ...
        while (this.trajectoryCache.size > maxEntries || this.trajectoryCacheBytes > maxBytes) {
            const oldest = this.trajectoryCache.entries().next().value;
            if (oldest === undefined)
                break;
    ...
    async evaluate(round, request, signal, reservation) {
        try {
            this.assertEvaluationRequest(round, request);
            ...
            try {
                ...
                throw new HitchEvaluationError(`Hitch emitted invalid JSON (${String(error)}); stderr: ${processResult.stderr.slice(-4000)}`, 'invalid_hitch_json');
            }
            let inspection;
            if (this.daemonMode) {
                inspection = await this.inspectEvaluation(reservation.evalId, round.workspaceRoot, signal, 'hitch_eval_inspect_failed');
            ...
            inspection ??= await this.inspectEvaluation(evidence.evalId, round.workspaceRoot, signal, 'hitch_eval_inspect_failed');
    ...
    prepareRerun(_round, _request, attempt, _selector) {
        if (!this.daemonMode)
            return undefined;
        return {
    ...
    rerunRoot(reservation) {
        if (reservation.provider !== 'hitch-cli' || !/^eval_[0-9a-f]{32}$/u.test(reservation.evalId)
            || !/^rerun_[0-9a-f]{32}$/u.test(reservation.rerunId))
            throw new TypeError('Hitch rerun reservation is invalid');
        const parameters = record(reservation.parameters, 'Hitch rerun parameters');
        if (typeof parameters.root !== 'string')
            throw new TypeError('Hitch rerun root is invalid');
        return parameters.root;
    }
    async cancelRerun(reservation) {
        ...
        if (result.exitCode !== 0)
            throw new HitchEvaluationError(`Hitch rerun cancellation failed: ${result.stderr.slice(-4000)}`, 'hitch_rerun_cancel_failed');
    ...
    async rerun(round, request, attempt, selector, signal, reservation) {
        ...
        if (this.daemonMode && reservation === undefined)
            throw new TypeError('Hitch daemon rerun requires a persisted rerun reservation');
        const root = reservation === undefined ? this.options.root : this.rerunRoot(reservation);
        if (reservation !== undefined && reservation.evalId !== attempt.evalId)
            throw new TypeError('Hitch rerun reservation does not match its source eval');
        try {
            ...
            const processResult = await this.run(args, round.workspaceRoot, signal);
            if (processResult.exitCode !== 0) {
                throw new HitchEvaluationError(`Hitch eval rerun failed for ${attempt.evalId}: ${processResult.stderr.slice(-4000)}`, 'hitch_eval_rerun_failed');
            }
            let parsed;
            try {
                parsed = JSON.parse(processResult.stdout);
            ...
            const inspection = await this.inspectEvaluation(attempt.evalId, round.workspaceRoot, signal, 'hitch_eval_rerun_inspect_failed', root);
            const result = record(inspection.result, 'Hitch repaired eval result');
            const evidence = this.parseResult(result, { stdout: JSON.stringify(result), stderr: '', exitCode: integer(result.exit_code, 'exit_code') }, request, this.daemonMode ? this.daemonEvaluationIdentity(round, request, inspection, identity) : identity, true);
            if (evidence.evalId !== attempt.evalId)
                throw new HitchEvaluationError('repaired evidence eval id changed', 'hitch_eval_identity_mismatch');
            ...
            if (reservation !== undefined) {
                try {
                    await this.cancelRerun(reservation);
                }
                catch (cleanupError) {
                    throw new EvaluationCleanupError(error, cleanupError);
                }
            }
            throw error;
        }
    }
    async inspectEvaluation(evalId, cwd, signal, failureCode, root = this.options.root) {
        ...
        let inspectionValue;
        try {
            inspectionValue = JSON.parse(inspect.stdout);
        }
        catch (error) {
    ...
    async cancelDaemonEvaluation(evalId, cwd, root) {
        const result = await this.run([
        ...
        if (cancelled.schema_version !== '1' || cancelled.eval_id !== evalId
            || !new Set(['cancelling', 'cancelled', 'succeeded', 'failed']).has(cancelled.status)) {
            throw new HitchEvaluationError('Hitch eval cancellation identity/status is invalid', 'invalid_hitch_result');
        }
    }
    daemonEvaluationIdentity(round, request, inspection, identity) {
    ...
    assertDaemonExecutionPolicy(value) {
        ...
        if (resources.setup !== undefined)
            this.parseResourceVector(resources.setup, 'Hitch eval setup resources');
    ...
    assertCompleteTrialSlots(inspection, evidence, request) {
        ...
        for (const trial of [...evidence.trials, ...evidence.invalidTrials]) {
            ...
            if (logicalAttempt === undefined || !Number.isSafeInteger(logicalAttempt)
                ...
                throw new HitchEvaluationError(`Hitch evidence attempt is outside frozen range 1..${attempts}: ${trial.taskName}#${String(trial.attempt)}`, 'invalid_hitch_result');
            }
        ...
        if (slots.size !== expectedTrials) {
            ...
            for (const task of tasks) {
                for (let attempt = 1; attempt <= attempts; attempt += 1) {
                    if (!slots.has(`${task}\0${attempt}`))
                        missing.push(`${task}#${attempt}`);
                }
            }
            throw new HitchEvaluationError(`Hitch evidence is missing frozen logical slots: ${missing.join(', ')}`, 'invalid_hitch_result');
        }
        return { id: benchmarkId, revision: benchmarkRevision };
    }
    effectiveConfigDigest(round, request) {
    ...
    invocationFingerprint(effectiveConfigDigest, hitchRuntimeIdentity) {
        return sha256(JSON.stringify({
            effectiveConfigDigest,
    ...
    daemonInvocationFingerprint(effectiveConfigDigest, directInvocationFingerprint) {
    ...
    async runtimeIdentity(signal) {
        signal?.throwIfAborted();
        const executablePath = await this.executablePath();
        const readExecutable = async () => readFile(executablePath).catch(error => {
            throw new HitchEvaluationError(`cannot fingerprint Hitch executable ${executablePath}: ${String(error)}`, 'hitch_version_check_failed');
        });
        ...
        if (executableDigest !== executableDigestBefore) {
            throw new HitchEvaluationError(`Hitch executable changed while its runtime identity was being validated: ${executablePath}`, 'hitch_version_check_failed');
        }
        return sha256(JSON.stringify({
            versionOutputDigest,
            executableDigest,
    ...
    async resolveExecutablePath() {
        ...
        for (const candidate of candidates) {
            try {
                ...
                if (error.code !== 'ENOENT') {
                    throw new HitchEvaluationError(`cannot resolve Hitch executable ${candidate}: ${String(error)}`, 'hitch_version_check_failed');
                }
            }
        }
        throw new HitchEvaluationError(`cannot resolve Hitch executable for runtime fingerprint: ${this.options.executable}`, 'hitch_version_check_failed');
    }
    parseResult(value, processResult, request, identity, allowFailedRunEvidence = false) {
        const result = record(value, 'Hitch result');
        if (result.schema_version !== '1')
            throw new HitchEvaluationError('unsupported Hitch eval schema', 'unsupported_hitch_schema');
        const evalId = string(result.eval_id, 'eval_id');
        if (!/^eval_[0-9a-f]{32}$/u.test(evalId))
            throw new HitchEvaluationError('invalid Hitch eval_id', 'invalid_hitch_result');
        ...
        if ((result.status !== 'succeeded' || exitCode !== 0) && !failedRunEvidence) {
            ...
            throw new HitchEvaluationError(`Hitch eval ${evalId} failed (${String(error.code ?? result.status)}): ${String(error.message ?? processResult.stderr).slice(-4000)}`, typeof error.code === 'string' ? error.code : 'hitch_eval_failed');
        }
        if (result.dataset !== request.dataset)
            throw new HitchEvaluationError('Hitch result dataset does not match the request');
        ...
        if (match?.[1] === undefined)
            throw new HitchEvaluationError('Hitch candidate does not contain a locked commit');
        ...
        if (Array.isArray(result.trials)) {
            const evidence = this.parseRunCenteredResult(result, summaryValue, evalId, request, actualCommit, revisionIdentity, identity, transport);
            if ((result.status === 'succeeded') !== (evidence.completeness === 'complete')) {
                throw new HitchEvaluationError('Hitch result status does not match run evidence completeness', 'invalid_hitch_result');
            }
            return evidence;
        }
        ...
        if (total <= 0 || completed !== total || errored !== 0 || cancelled !== 0) {
            throw new HitchEvaluationError(`Hitch eval has incomplete trials: total=${total}, completed=${completed}, errored=${errored}, cancelled=${cancelled}`, 'hitch_infrastructure_failure');
        }
        ...
        if (trials.length !== total)
            throw new HitchEvaluationError('Hitch trial count does not match summary.n_trials');
    ...
    parseRunCenteredResult(result, summaryValue, evalId, request, actualCommit, revisionIdentity, identity, transport) {
        const total = integer(summaryValue.n_trials, 'summary.n_trials');
        const completed = integer(summaryValue.n_completed, 'summary.n_completed');
        const invalid = integer(summaryValue.n_invalid, 'summary.n_invalid');
        const parsed = this.parseRunTrials(result.trials);
        if (total <= 0 || parsed.length !== total)
            throw new HitchEvaluationError('Hitch trial count does not match summary.n_trials');
        ...
        if (completed !== valid.length || invalid !== invalidObservations.length || completed + invalid !== total) {
            throw new HitchEvaluationError(`Hitch observation counts are inconsistent: total=${total}, completed=${completed}, invalid=${invalid}`, 'invalid_hitch_result');
        }
        const trials = valid.map(trial => ({
        ...
        const invalidTrials = invalidObservations.map(trial => ({
            taskName: trial.taskName,
            trialName: trial.trialName,
            runId: trial.runId,
            attempt: trial.attempt,
    ...
    parseRunTrials(value) {
        if (!Array.isArray(value))
            throw new HitchEvaluationError('trials must be an array', 'invalid_hitch_result');
        return value.map((item, index) => {
            const trial = record(item, `trials[${index}]`);
            const runId = string(trial.run_id, `trials[${index}].run_id`);
            if (!/^run_[0-9a-f]{32}$/u.test(runId))
                throw new HitchEvaluationError(`trials[${index}].run_id is invalid`, 'invalid_hitch_result');
            ...
            const scores = observation === 'valid'
                ? (trial.scores === undefined
                    ? { totalScore: reward, normalization: 'legacy-reward' }
            ...
            if (attempt <= 0)
                throw new HitchEvaluationError(`trials[${index}].attempt must be positive`, 'invalid_hitch_result');
            return {
    ...
    parseTrials(value) {
        if (!Array.isArray(value))
            throw new HitchEvaluationError('summary.trials must be an array', 'invalid_hitch_result');
        return value.map((item, index) => {
            ...
            const rewards = {};
            for (const [name, reward] of Object.entries(rewardsValue))
                rewards[name] = finite(reward, `trial reward ${name}`);
    ...
    async run(args, cwd, signal, maxOutputBytes = this.options.maxOutputBytes) {
        signal.throwIfAborted();
        const executablePath = await this.executablePath();
        signal.throwIfAborted();
        return new Promise((resolvePromise, reject) => {
            ...
            child.once('exit', (code, childSignal) => {
                ...
                if (code === null)
                    return reject(new HitchEvaluationError(`Hitch exited from signal ${childSignal ?? 'unknown'}`));
                resolvePromise({ stdout: stdoutChunks.join(''), stderr: stderrChunks.join(''), exitCode: code });
            });
        });
    }
}
//# sourceMappingURL=hitch-cli.js.map