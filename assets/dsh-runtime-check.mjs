...
try {
  ...
  if (resources.length === 0) {
    ...
    expectedProvider = new skillFilesystem.FileSystemSkillProvider(ctx, {
    ...
    const observation = await expectedProvider.list({ cwd: process.cwd() })
    ...
    const actual = await skills.list({ cwd: process.cwd() })
    const reads = []
    for (const entry of expected) {
      const found = actual.find(item => item.name === entry.name)
      const full = await skills.get(entry.name, { cwd: process.cwd() })
      if (!found || !full || resolve(full.path ?? '') !== resolve(entry.path)) {
        throw new Error(`SKILL_NOT_DISCOVERED: ${entry.name} at ${relative(request.repository, entry.path)}; check loader registration and skill directory`)
      }
      const intended = await expectedProvider.get(entry, { cwd: process.cwd() })
    ...
    for (const { entry, full, intended } of reads) {
      ...
      const result = await tools.execute({ name: 'skill', arguments: { name: entry.name },
        callId: `gear-smoke-${checked}`, signal: new AbortController().signal })
  ...
  try {
    await Promise.race([
      (async () => { await expectedProvider?.dispose(); await ctx?.fiber.dispose() })(),
