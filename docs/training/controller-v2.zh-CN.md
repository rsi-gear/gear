...
代码入口已接入公开 CLI，真实 SSH、Harbor 工具轨迹和单卡正常训练已有诊断证据。本轮继续验证单卡故障恢复、信息隔离和 runtime 认证；远程 Harbor / Docker 主机及双卡回归延后。以下说明配置合同，具体通过范围以 [实施记录](execution-placement-status.zh-CN.md) 为准，未完成的 runtime 不能用于正式准入。

## 控制端

Gear、Hitch、任务快照与 canonical/verifier 证据保留在控制端。控制端 Python 用于 CAS materialize，不要求装 CUDA。示例路径需要替换；`vast-debug` 是用户 SSH 配置中的 Host 别名。
...
初始 HF 模型已在节点上时，使用 `gear-refine training seal-hf-node /workspace/models/parent --config CONTROLLER.json`，在节点完成 safetensors/有限数值检查并回传描述。模型节点缺文件、代际漂移或回执不完整均会拒绝完成收集。节点间迁移使用 Vast 实例间 copy，校验后再删除不可用旧实例；未完成验收的可用实例只停止并保留。

## 模型节点

节点安装本分支 Gear Python 包以及锁定的 Slime / Megatron / SGLang / torch 运行时。纯模型节点无需 Hitch CLI 或 Docker。`node.json` 示例：

```json
{
  "schemaVersion": 2,
  "nodeId": "vast-debug-model",
...
费用按实验内原评估 key 的累计最大值计增量，跨训练 run 恢复不会重复扣减已计入的时间。旧状态缺少 `chargedGpuSeconds` 时读取原 evidence 成本，迟到结果不能降低较新的费用；有效结果仍可保留。控制端观察依赖调用 `advance`，不是后台定时计费器；节点账本在控制端离线时继续累计。真实故障与 GPU 验收以实施记录为准。
...
远程 worker 重新注册或撤销后，旧 generation 的在途状态提交和制品发布会在控制端最终写入时被拒绝。HTTP 请求使用其 bearer 实际认证的 generation，不能靠稍后送达的正文改变身份；制品传输不会阻塞 worker 心跳。调度/恢复发现 generation 已变即隔离旧 execution lease，保留原 resource epoch 和未释放 offer 的预留。新 worker 的零 allocation 不能代替旧资源清理证明，跨 generation 清理仍是后续交接工作。

同 generation 的 Harbor worker 重启现在执行原任务清理：公开 runner 在接受前持久化 offer/nonce、lease/epoch、worker/主机启动/root/Docker 引擎身份，candidate 和 verifier 通过身份落盘后才允许启动的 supervisor 执行。新 worker 只在原进程归属与环境身份可核验时停止原 Harbor 进程组、清理并复查 Docker 资源，随后回报原任务失败并完成原 epoch 释放；不会重新执行已接受候选。缺失记录、原 worker 仍存活、引擎/启动身份变化或资源未清空时保留预占。该路径已有真实 worker SIGKILL/重启和公开 CLI 验证，Docker、Harbor、模型及 GPU 仍为 fixture；主机重启、跨 generation 和真实云端故障交接继续待验收。

凭据轮换或撤销时，旧 worker 客户端识别控制端明确的认证/代际拒绝，中断所有在途控制请求并取消执行器，执行器结束后尝试本地清理，再以退出码 11 结束；清理失败会报告错误并保留未确认占用。daemon 短时不可达和模型服务自身的认证错误不会误触发这一分支。控制端继续保留旧资源预占；旧 worker 自行退出和本地清理记录还不足以供新 generation 关闭旧 lease，需要由下面的独立跨 generation 回执核销。

公开 Harbor worker 现在通过独立 v2 接受/进程授权接口，把原 generation 的资源归属和 supervisor 启动身份保存到 daemon。接受凭据绑定原 offer、任务/输入、lease/resource epoch、worker 进程、主机启动、root 与 Docker engine；supervisor 需先落盘本地身份，再取得控制端对同一进程的授权，才能启动 candidate 或 verifier。原 v1 offer/回执格式不变，既有 v1 已接受任务不能事后补造这份凭据。最终写入与 worker 凭据轮换共用 generation 保护，HTTP 响应丢失后的重试保留原时间、事件序号和身份。实际 CLI 中断测试已核对控制端保存的是原 worker 与 Harbor supervisor；独立清理回执与旧预占核销见下一段，真实 Docker、跨主机及 GPU 验收继续等待对应环境。

上述跨 generation 清理现已接入同一主机启动周期的公开 worker：新 generation 获取绑定原 admission 的限时 challenge，核对本地 journal、原 worker/supervisor、root、boot 和 Docker engine，停止原授权进程并确认进程组与原 lease 的容器/网络/卷全部清空。独立回执由新 generation 认证，控制端据此写入 schema 3 的 lease 释放确认；原 offer、v1 回执、执行身份和隔离 epoch 均保留，不能恢复旧模型路由或重新执行旧候选。后台清理期间心跳继续；回包丢失或回执已落盘但 lease 尚未更新时可以幂等重试。真实 CLI 轮换/撤销后已验证新 worker 核销原预占、候选启动仍为一次，但 Docker/Harbor/模型为 fixture。主机重启、真实跨主机/Docker 和 GPU 设备交接仍在后续验收范围内。

主机重启恢复也已接入：新任务以 schema 3 ownership 保存原主机与启动身份，本地 journal 为 schema 2。原 root/主机/Docker engine 一致且 boot 不同时，跳过旧 PID/进程组检查与终止，清理原 lease 的 Docker 资源并复查；跨 generation 返回 schema 3 receipt，明确前一启动周期已结束，而不声称观察了旧 PID。旧 ownership/journal 不升级，缺少原始主机身份仍不能跨 boot 释放；另一主机、引擎漂移或残留资源继续拒绝。本地测试保留了模拟复用 PID 的真实进程，并检查真实 HTTP 回执和预占核销；boot 变化及 Docker 为 fixture，真实主机重启/GPU 验收仍待云端执行。
