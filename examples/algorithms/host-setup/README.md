...
在仓库中执行 `npm run test:algorithm:package` 检查安装后的 npm 公开入口与独立 Python wheel。包外 [`recorded-check.mjs`](recorded-check.mjs) 在已安装包的外部项目中生成合成 Git/编译数据集、`host.settings.json`、`host.mjs`、`model.mjs` 和 Campaign JSON，使用**已安装** CLI 与干净 venv 的 wheel 执行 `algorithm check`，证明公开物理 provider 可装配；该录制 adapter **不会**发起模型请求或 Hitch evaluation。验证器会自动传入 `GEAR_ALGORITHM_PACKAGE_PYTHON` 和 `GEAR_ALGORITHM_PACKAGE_CLI`；若单独运行该脚本，须将这两个变量分别指向已安装 wheel 的 Python 与已安装 npm 包的 `lib/cli.js`。生产 adapter 的 `check` 可能执行模型元数据查询，不能由录制夹具推断为离线。完整 RHO/AHE/Evo 的 Python recipe、受限 DSH 工具、Git 提交、录制 Hitch CLI 与恢复由 `tests/unit/algorithm-default-{rho,ahe,evo}.spec.ts` 离线覆盖。真实模型、Hitch daemon、GPU 和人工作者环境尚未验收；这些测试不能推断科学效果。
